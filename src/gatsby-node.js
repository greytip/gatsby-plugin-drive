...
exports.onPreBootstrap = ({ graphql, actions }, { folderId, keyFile, key, destination, exportGDocs, exportMimeType, exportMiddleware, pageSize = 100 }) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let file of filesToDownload) {
      ...
      if (file.mimeType === FOLDER) {
        ...
        promises.push(recursiveFolders(files, `${parent}/${file.name}`, token, destination, pageSize));
      } else {
...
const fileExtensionsByMime = new Map([['text/html', '.html'], ['application/zip', '.zip'], ['text/plain', '.txt'], ['application/rtf', '.rtf'], ['application/vnd.oasis.opendocument.text', '.odt'], ['application/pdf', '.pdf'], ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', '.docx'], ['application/epub+zip', '.epub']]);
